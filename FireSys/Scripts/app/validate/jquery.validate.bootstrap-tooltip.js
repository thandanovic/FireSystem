            ...
            defaultShowErrors: function () {
                ...
                $.each(this.errorList, function (index, value) {
                    ...
                    if (bsMajorVer === 3 && bsMinorVer >= 3) {
                        ...
                        //Update message only if its different from new one
                        if ($currentElement.data('bs.tooltip') !== undefined && $currentElement.data('bs.tooltip').options.title === value.message) {
                            return false;
                        }
                        ...
                        $(value.element).removeClass(_this.settings.validClass)
                                        ...
                                        .tooltip('show').data('bs.tooltip').tip().addClass('tooltip-error');
                    } else {
                        $(value.element).removeClass(_this.settings.validClass)
                                        .addClass(_this.settings.errorClass)
                                        .tooltip(bsMajorVer === 4 ? 'dispose' : 'destroy')
                                        .tooltip(_this.applyTooltipOptions(value.element, value.message))
                                        .data('bs.tooltip').tip().addClass('tooltip-error')
                                        .tooltip('show');
                    }
                ...
                $.each(_this.validElements(), function (index, value) {
                    $(value).tooltip(bsMajorVer === 4 ? 'dispose' : 'destroy')
